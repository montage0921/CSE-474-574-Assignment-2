...
if __name__ == "__main__":
    ...
    # Set the number of nodes in the input and output units
    n_input = train_data.shape[1]
    n_class = 10

    # Define the ranges for lambda and hidden units
    hidden_units = [1, 4, 8, 12, 16, 20]
    lambda_values = np.arange(0, 61, 5)
    ...
    for lambdaval in lambda_values:
        for n_hidden in hidden_units:
            ...
            accuracy = 100 * np.mean((predicted_label == test_label).astype(float))

            # Store the accuracy
            acc_dict[(lambdaval, n_hidden)] = accuracy

    # Prepare data for visualization
    accuracies = np.array([v for v in acc_dict.values()]).reshape(len(lambda_values), len(hidden_units))

    # Plot heatmap
    plt.figure(figsize=(10, 6))
    sns.heatmap(accuracies, xticklabels=hidden_units, yticklabels=lambda_values, annot=True, fmt=".2f", cmap="viridis")
    plt.xlabel("Number of Hidden Units")
    plt.ylabel("Lambda (Regularization Parameter)")
    plt.title("Test Accuracy for Different Lambda and Hidden Units")
    plt.show()
